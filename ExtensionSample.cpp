...
// Reads values from registers passed to it, the first is the duration, the serond is the tone in Hz.
// The instruction bits are:
//
//		1  1  1  1  1  1
...
LC_API void ExecuteReservedOpcode3(GetValueMethod GetValue, SetValueMethod SetValue)
{
	...
	// Get the source registers for the tone and duration
	int durReg = (instruction & 0x0E00) >> 9;
	int toneReg = (instruction & 0x01c0) >> 6;
	// Convert the registers number to a constant that can be passed to GetValue
	int setLocation = 0;
	switch (durReg)
	{
	...
	BITS duration = 0, tone = 0;

	if(GetValue(setLocation, duration) != LCEXT_SUCCESS)
		return;
	...
	if(GetValue(setLocation, tone) != LCEXT_SUCCESS)
		return;
	int retStatus = 0;
	{
		LC3TimeTable sTable;
		sTable.length = duration;
		sTable.tone = tone;
		retStatus = soundPtr->AddTimeTable(sTable);
		//SoundData.tableIndex++; 
	}
	short PSRMod = 0;
